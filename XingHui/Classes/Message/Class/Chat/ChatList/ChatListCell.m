    ...
    reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        // Initialization code
        _timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH-80, 7, 80, 16)];
        ...
        _placeholderImage =[UIImage imageNamed:@"chatListCellHead"];
    }
    return self;
}
...
//      [[XHUserHelper share] getUserFromTell:_name complete:^(XHUser *users){
//           [self.textLabel setTextWithUsername:users.userRealName];
//          if (users.userpic.length>0 ) {
//              [self.imageView sd_setImageWithURL:[NSURL URLWithString:users.userpic]];
//          }
//          else
//          {
//                _placeholderImage=[UIImage imageFromString:users.userRealName size:self.imageView.frame.size fontsize:30 top:5];
...
-(void)setHeadPic:(XHUser *)user
{
    [self.imageView sd_setImageWithURL:[NSURL URLWithString:user.userpic] placeholderImage:_placeholderImage];
    [self.textLabel setText:user.userRealName];
}
