...
@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, LocationViewDelegate, EMCDDeviceManagerDelegate,EMCallManagerDelegate,PassValueDelegate>
{
...
{
     self.navtitle=realname;
}
-(void)setNavTitles:(id)title
{
    ...
    else
    {
        ...
        [nickDic setObject:title forKey:_chatter];
        self.navtitle=title[@"nick"];
        
        [[XHUserHelper share] getUserFromTell:_chatter complete:^(XHUser *users){
            user = users;
            //  [self setTitle:user.userRealName];
            self.navtitle=user.userRealName;
        }];
...
- (void)viewDidLoad
{
  ...
  //  NSString *titles= [[XHUserHelper share] getUserFromTell:_chatter].username;
...
//        [detailButton setImage:[UIImage imageNamed:@"group_detail@2x"] forState:UIControlStateNormal];
...
//        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:detailButton];
    ...
    {
        [self.rightButton setImage:[UIImage imageNamed:@"group_detail@2x"] forState:UIControlStateNormal];
    }
    ...
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (!model.isSender) {
                ...
                if (nickDic[model.username]) {
                    
                    if (!model.headImageURL) {
                        if (![[[NSURL URLWithString:nickDic[model.username][@"pic"]] ToString] isEqualToString:@""]) {
                            model.headImageURL = [NSURL URLWithString:nickDic[model.username][@"pic"]];
                        }
                    }
                   else
                   {
                       if (![[nickDic[model.username][@"pic"] ToString] isEqualToString:model.headImageURL.absoluteString]) {
                           NSDictionary *dic = @{@"nick":model.nickName,@"pic":model.headImageURL.absoluteString};
                           [nickDic setObject:dic forKey:model.username];
                           LKDBHelper *dbhelper =[[LKDBHelper alloc] initWithDBName:@"XINGHUI"];
                           [dbhelper updateToDB:[XHUser class] set:[NSString stringWithFormat:@"userpic='%@'",model.headImageURL.absoluteString] where:[NSString stringWithFormat:@"username ='%@'",model.username]];
                       }
                      
                      
                   }
                    
                    model.nickName = nickDic[model.username][@"nick"];
                }
                else
                {
            ...
            {
                model.headImageURL = [NSURL URLWithString:[XHDefaultUser sharedUser].userpic];
            }
...
- (void)chatHeadImagePressed:(MessageModel *)model
{
    if(!model.isSender)
    {
        if(self.isChatGroup)
        {
              XHPersonDetailViewController *person = [[XHPersonDetailViewController alloc] init];
...
- (void)moreViewAudioCallAction:(DXChatBarMoreView *)moreView
{
    // 隐藏键盘
    [self keyBoardHidden];
    NSDictionary *dic =@{@"chatter":self.chatter,
                         @"name":[[nickDic objectForKey:self.chatter] objectForKey:@"nick"],
                         @"pic":[[nickDic objectForKey:self.chatter] objectForKey:@"pic"],
                         @"type":[NSNumber numberWithInt:eCallSessionTypeAudio]
                         };
    [[NSNotificationCenter defaultCenter] postNotificationName:@"callOutWithChatter" object:dic];
}
...
- (void)moreViewVideoCallAction:(DXChatBarMoreView *)moreView
{
    ...
    NSDictionary *dic =@{@"chatter":self.chatter,
                         @"name":[[nickDic objectForKey:self.chatter] objectForKey:@"nick"],
                         @"pic":[[nickDic objectForKey:self.chatter] objectForKey:@"pic"],
                         @"type":[NSNumber numberWithInt:eCallSessionTypeVideo]
                         };
    [[NSNotificationCenter defaultCenter] postNotificationName:@"callOutWithChatter" object:dic];
}
...
-(void)didSelectAtMember
{
    XHSelectAtVC *selectAt =[[XHSelectAtVC alloc] initWithGroupId:_chatter];
    selectAt.passdelegate =self;
    UINavigationController *nav =[[UINavigationController alloc] initWithRootViewController:selectAt];
    [self.navigationController presentViewController:nav animated:YES completion:nil];
}
-(void)passbackObject:(id)object sender:(NSString *)sender
{
    XHUser *usera=object;
    _chatToolBar.inputTextView.text=[NSString stringWithFormat:@"%@@%@ ",_chatToolBar.inputTextView.text,usera.userRealName];
}
/**
 *  按下录音按钮开始录音
 */
...
- (void)loadMoreMessagesFrom:(long long)timestamp count:(NSInteger)count append:(BOOL)append
{
    ...
    dispatch_async(_messageQueue, ^{
        ...
        if ([messages count] > 0) {
            ...
            for (EMMessage *messga in messages) {
                NSString *uname =messga.groupSenderName;
                if(!self.isChatGroup)
                    uname=messga.conversationChatter;
                [[XHUserHelper share] getUserFromTell:uname complete:^(XHUser *usert){
                    NSDictionary *dic =@{@"nick":usert.userRealName,@"pic":usert.userpic};
                    [nickDic setObject:dic forKey:usert.username];
                }];
...
-(NSMutableArray *)formatMessage:(EMMessage *)message
{
    ...
    if ([_delelgate respondsToSelector:@selector(nickNameWithChatter:)]) {
        ...
        model.nickName = [message.ext objectForKey:@"nick"];
    }
    model.nickName = [message.ext objectForKey:@"nick"];
    model.headImageURL = [NSURL URLWithString:[message.ext objectForKey:@"pic"]];
...
{
    NSDictionary *ext = @{@"nick":[XHDefaultUser sharedUser].userRealName,@"pic":[XHDefaultUser sharedUser].userpic};
