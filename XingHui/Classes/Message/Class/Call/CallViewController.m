...
- (instancetype)initWithSession:(EMCallSession *)session
                     isIncoming:(BOOL)isIncoming nick:(NSDictionary *)nick
{
    ...
    if (self) {
        ...
        self.userNick =nick[@"name"];
        self.userPic = nick[@"pic"];
...
{
    return [self initWithSession:session isIncoming:isIncoming nick:nil];
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
- (void)_setupSubviews
{
    ...
    [_headerImageView sd_setImageWithURL:[NSURL URLWithString:self.userPic] placeholderImage:[UIImage imageNamed:@"chatListCellHead"]];
    ...
    [_answerButton setTitle:@"接受" forState:UIControlStateNormal];
                           ...
                           error:(EMError *)error
{
    ...
    if(error){
        ...
        [self _insertMessageWithStr:@"呼叫失败"];
    ...
    else if (callSession.status == eCallSessionStatusAccepted)
    {
        if (callSession.connectType == eCallConnectTypeRelay) {
            _statusLabel.text =@"请稍后...";
        }
        else if (callSession.connectType == eCallConnectTypeDirect){
            _statusLabel.text = @"请直接通话";
        }
        else{
            _statusLabel.text =@"请讲";
        }
        _timeLength = 0;
