...
@property (nonatomic,strong)NSMutableDictionary *userDetailArr;
- (void)dissolveAction;
- (void)clearAction;
- (void)exitAction;
...
- (void)viewDidLoad
{
    ...
    [self setTitle:@"群组"];
    _userDetailArr =[NSMutableDictionary new];
    UIButton *backButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 44, 44)];
    [backButton setImage:[UIImage imageNamed:@"bt_return"] forState:UIControlStateNormal];
        ...
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [self.dataSource count]) {
                ...
                [[XHUserHelper share] getUserFromTell:username complete:^(XHUser *user){
                     ...
                     contactView.image =[NSURL URLWithString:user.userpic];
                     contactView.remark = user.userRealName;
                    contactView.tag =index;
                    [_userDetailArr setObject:user forKey:[NSString stringWithFormat:@"%ld",(long)contactView.tag]];
                }];
                [contactView addTarget:self action:@selector(toPersonVC:)];
...
-(void)toPersonVC:(UITapGestureRecognizer *)tapgest
{
 
    XHPersonDetailViewController *person =[[XHPersonDetailViewController alloc] init];
    person.user =[_userDetailArr objectForKey:[NSString stringWithFormat:@"%ld",tapgest.view.tag]];
