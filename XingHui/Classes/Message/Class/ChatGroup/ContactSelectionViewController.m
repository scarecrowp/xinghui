        ...
        for (NSString *username in _blockSelectedUsernames) {
            ...
            if (tmpArray && [tmpArray count] > 0) {
                for (int i = 0; i < [tmpArray count]; i++) {
                    XHUser *buddy = [tmpArray objectAtIndex:i];
...
- (EMSearchDisplayController *)searchController
{
    if (_searchController == nil) {
        ...
        [_searchController setCellForRowAtIndexPathCompletion:^UITableViewCell *(UITableView *tableView, NSIndexPath *indexPath) {
            ...
            XHUser *buddy = [weakSelf.searchController.resultsSource objectAtIndex:indexPath.row];
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:buddy.userpic] placeholderImage:[UIImage imageNamed:@"chatListCellHead.png"]];//sd_setImageWithURL:[NSURL URLWithString:buddy.userpic] placeholderImage:[UIImage imageNamed:@"chatListCellHead.png"]tggthygh
            cell.textLabel.text = buddy.userRealName;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    XHUser *buddy = [[_dataSource objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
  //  cell.imageView.image = [UIImage imageNamed:@"chatListCellHead.png"];
    [[XHUserHelper share] getUserFromTell:buddy.username complete:^(XHUser *user){
        cell.textLabel.text =user.userRealName;
    }];
   // cell.username = buddy.username;
    if (buddy.userpic) {
        if ([[buddy.userpic ToString] isEqualToString:@""]) {
            [cell.imageView setImageFromString:buddy.userRealName fontsize:30 paddintTop:10];
        }
        else
        {
            [cell.imageView sd_setImageWithURL:[NSURL URLWithString:buddy.userpic]];
        }
    }
    else
    {
...
- (void)loadDataSource
{
    ...
    [[UserManager share] getOLFriendComplete:^(NSMutableArray *userArr){
        for (XHUser *buddy in userArr) {
...
//    [[EaseMob sharedInstance].chatManager asyncFetchBuddyListWithCompletion:^(NSArray *arr,EMError *error){
