...
- (void)applyCellAddFriendAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        else */if (applyStyle == ApplyStyleJoinGroup)
        {
            ...
            [[XHNetWork sharedNetWork] Get:[NSString APIURLString:@"addfriend.ashx"] param:@{@"fname":entity.applicantUsername} complete:^(NSDictionary *dic){
                LKDBHelper *dbhelper =[[LKDBHelper alloc] initWithDBName:@"XINGHUI"];
                XHUser *user =[[XHUser alloc] initWithDic:[[dic objectForKey:@"content"] objectAtIndex:0]];
                user.myfriend=1;
