...
@interface MainViewController () <UIAlertViewDelegate, IChatManagerDelegate, EMCallManagerDelegate>
{
    ...
    XHHomeViewController *homeVC;
    ChatListViewController *messageVC;
    XHMeViewControl *meVC;
    XHFoundViewControl *findVC;
...
- (void)setupSubviews
{
   ...
   // self.tabBar.selectionIndicatorImage = [[UIImage imageNamed:@"tabbarSelectBg"] stretchableImageWithLeftCapWidth:25 topCapHeight:25];
    ...
    homeVC = [[XHHomeViewController alloc] init];
    UINavigationController *nav0 = [[UINavigationController alloc] initWithRootViewController:homeVC];
    homeVC.tabBarItem = [[XHTabBarItem alloc] initWithTitle:@"幸会" image:@"home" tag:0 selectImg:@"home_select"];
    
    messageVC=[[ChatListViewController alloc] init];
    ...
    messageVC.tabBarItem = [[XHTabBarItem alloc] initWithTitle:@"消息" image:@"message" tag:1 selectImg:@"message_select"];
    nav1.hidesBottomBarWhenPushed=YES;
    findVC= [[XHFoundViewControl alloc] init];
    UINavigationController *nav2 = [[UINavigationController alloc] initWithRootViewController:findVC];
    
    findVC.tabBarItem = [[XHTabBarItem alloc] initWithTitle:@"发现" image:@"find" tag:2 selectImg:@"find_select"];
    
    meVC= [[XHMeViewControl alloc] init];
    UINavigationController *nav3 = [[UINavigationController alloc] initWithRootViewController:meVC];
    
    meVC.tabBarItem = [[XHTabBarItem alloc] initWithTitle:@"我" image:@"me" tag:3 selectImg:@"me_select"];
...
- (void)callOutWithChatter:(NSNotification *)notification
{
    ...
    if ([object isKindOfClass:[NSDictionary class]]) {
        ...
        NSString *nick =[object objectForKey:@"name"];
        NSString *pic = [object objectForKey:@"pic"];
        ...
        if (callSession && !error) {
            callSession.userNick =nick;
            callSession.userPic = pic;
            [[EaseMob sharedInstance].callManager removeDelegate:self];
            
            CallViewController *callController = [[CallViewController alloc] initWithSession:callSession isIncoming:NO nick:object];
...
-(void)didViewControlTransfor:(UIViewController *)controll
{
    ...
    [homeVC.navigationController pushViewController:controll animated:YES];
}
...
#warning 在后台不能进行视频通话
            ...
            if (!isShowPicker){
                [[EaseMob sharedInstance].callManager removeDelegate:self];
                 [[XHUserHelper share] getUserFromTell:callSession.sessionChatter complete:^(XHUser *user){
                     NSDictionary *dic = @{@"name":user.userRealName,@"pic":user.userpic};
                     CallViewController *callController = [[CallViewController alloc] initWithSession:callSession isIncoming:YES nick:dic];
                     callController.modalPresentationStyle = UIModalPresentationOverFullScreen;
                     [self presentViewController:callController animated:NO completion:nil];
...
- (void)jumpToChatList
{
    ...
    else if(messageVC)
    {
        ...
        [self setSelectedViewController:messageVC];
    }
}
